...
It covers the survey results collected from 40 representatives of medium 
and large Latvian enterprises with formal PMO structures, along with 
findings from one structured expert interview. The chapter is organized 
into five sections: sample characteristics, descriptive statistics, 
hypothesis testing, expert interview findings, and a chapter summary.

\section{Sample Characteristics}
\label{sec:sample}

A total of 40 valid survey responses were collected, which meets the 
target of 30--50 responses set out in the research methodology. The 
respondents represented organizations from various industries and sizes, 
providing a diverse and representative sample for the analysis.

\subsection{Organization Size}
More than half of the respondents came from medium-sized enterprises. 
...
the reality of the Latvian business landscape, which is dominated by 
...
the validity of the findings by ensuring that the results are not 
...
highest level of integration maturity, and another 15\% (n=6) are still 
at the most basic level. This pattern suggests that while many Latvian 
enterprises have moved beyond completely manual integration, full 
platform-level integration remains rare.
...
All three constructs show moderate mean scores, which suggests that 
the sampled organizations have achieved a moderate but not yet high 
level of performance in these areas. Governance standardization 
recorded the highest mean (M=3.64), while data quality had the lowest 
(M=3.32). The standard deviations indicate a reasonable spread of 
responses, reflecting genuine variation in IS integration practices 
across the sample.
...
Expert 1 is an experienced project management professional working 
in a PMO environment. Their organization operates what they described 
as a hybrid-integrated IS ecosystem, using tools such as ServiceNow 
or Planview for strategic portfolio management, Jira for agile 
execution, and Microsoft Project Online for waterfall-style schedules. 
These systems are connected through APIs and middleware solutions, 
placing the organization at a high integration maturity level 
(Level 3--4 of the IMM).

Even at this high level of integration, Expert 1 identified three 
recurring challenges. First, data latency — while systems are 
connected, brief delays between execution tools and the portfolio 
layer can result in stakeholders viewing slightly outdated reports. 
Second, semantic misalignment — different departments define concepts 
like progress differently, which requires ongoing reconciliation 
between technical and financial systems. Third, legacy resistance — 
older ERP systems used by Finance are difficult to connect with 
modern cloud-based tools, sometimes requiring manual data exports 
as a workaround.
